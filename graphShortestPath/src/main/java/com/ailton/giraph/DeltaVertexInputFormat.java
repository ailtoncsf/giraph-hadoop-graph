...
* https://gist.github.com/sar-vivek
*/
 ...
 * {@link DeltaVertexWritable} vertex values and <code>float</code> out-edge
 * weights, and <code>double</code> message types, specified in JSON format.
 */
public class DeltaVertexInputFormat extends TextVertexInputFormat<LongWritable, DeltaVertexWritable, FloatWritable> {
	...
	/**
	 * VertexReader that features <code>DeltaVertexWritable</code> vertex values
	 * and <code>float</code> out-edge weights. The files should be in the
	 ...
	 */
	class JsonLongDeltaFloatDoubleVertexReader
			extends TextVertexReaderFromEachLineProcessedHandlingExceptions<JSONArray, JSONException> {
		...
		@Override
		protected DeltaVertexWritable getValue(JSONArray jsonVertex) throws JSONException, IOException {
			return new DeltaVertexWritable(jsonVertex.getDouble(1), Long.MAX_VALUE, 0);
		}

		@Override
		protected Iterable<Edge<LongWritable, FloatWritable>> getEdges(JSONArray jsonVertex)
		...
		@Override
		protected Vertex<LongWritable, DeltaVertexWritable, FloatWritable> handleException(Text line,
