 ...
 */
public class LongDoubleNullTextInputFormat extends TextVertexInputFormat<LongWritable, DoubleWritable, NullWritable>
	...
	public class LongDoubleNullDoubleVertexReader
			extends TextVertexInputFormat<LongWritable, DoubleWritable, NullWritable>.TextVertexReader {
		...
		public Vertex<LongWritable, DoubleWritable, NullWritable> getCurrentVertex()
			...
			List<Edge<LongWritable, NullWritable>> edges = Lists.newArrayListWithCapacity(tokens.length - 1);
			for (int n = 1; n < tokens.length; n++) {
				edges.add(EdgeFactory.create(new LongWritable(Long.parseLong(tokens[n])), NullWritable.get()));
			}
