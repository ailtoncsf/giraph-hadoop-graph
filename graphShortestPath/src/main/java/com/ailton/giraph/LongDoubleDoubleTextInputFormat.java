 ...
 * neighbor2 ...
 */
public class LongDoubleDoubleTextInputFormat extends TextVertexInputFormat<LongWritable, DoubleWritable, DoubleWritable>
	...
	public class LongDoubleDoubleDoubleVertexReader
			extends TextVertexInputFormat<LongWritable, DoubleWritable, DoubleWritable>.TextVertexReader {
		...
		public Vertex<LongWritable, DoubleWritable, DoubleWritable> getCurrentVertex()
			...
			List<Edge<LongWritable, DoubleWritable>> edges = Lists.newArrayListWithCapacity(tokens.length - 1);
			float weight = 1.0f / (tokens.length - 1);
			for (int n = 1; n < tokens.length; n++) {
				edges.add(EdgeFactory.create(new LongWritable(Long.parseLong(tokens[n])), new DoubleWritable(weight)));
			}
